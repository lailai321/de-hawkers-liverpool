...
-- holidays table (created separately) also needs RLS enabled.
-- All app access goes through the service role key (bypasses RLS), but the
-- public /api/holidays endpoint just mirrors the date list, so allow public read too.
